...
*/
function clearAll() {
  // Holt alle relevanten Content-Container
  const containers = document.querySelectorAll('.meme-content, .joke-content, .quote-content, .riddle-content');

  // Leert ALLE jeweiligen Container direkt (setzt den Inhalt wirklich auf leer)
...
// Zähler für Meme-Klicks
window.memeClickCount = 0;
window.memeLocked = false;

// Das finale Bild nach 10 Klicks (Rick Roll)
const finalMemeUrl = 'https://i.imgflip.com/2/1bij.jpg';

function showMeme() {
  // Wenn gesperrt, nichts mehr tun
  if (window.memeLocked) {
    return;
  }

  clearAll();
  window.memeClickCount++;

  // Nach 10 Klicks: zeige das finale Bild und sperre die Funktion
  if (window.memeClickCount > 10) {
    window.memeLocked = true;
    const container = document.querySelector('.meme-content');
    const newimg = document.createElement('img');
    newimg.setAttribute('src', finalMemeUrl);
    container.appendChild(newimg);
    const msg = document.createElement('p');
    msg.classList.add('game-over-text');
    msg.textContent = 'You have been Rick Rolled! No more memes for you.';
  ...
  // Verhindere dasselbe Meme zweimal hintereinander
  if (window.lastMemeUrl === randomMemeUrl) {
    let tries = 0;
    while (randomMemeUrl === window.lastMemeUrl && tries < 10) {
  ...
  container.appendChild(newimg);
  window.lastMemeUrl = randomMemeUrl;
}
...
function showJoke() {
  clearAll();
  let randomJokeText = getRandomData('jokes');

  // Verhindere denselben Joke zweimal hintereinander
  if (window.lastJokeText === randomJokeText) {
    let tries = 0;
    while (randomJokeText === window.lastJokeText && tries < 10) {
      randomJokeText = getRandomData('jokes');
      tries++;
    }
  }
  ...
  window.lastJokeText = randomJokeText;
}
...
function showQuote() {
  ...
  const newP = document.createElement('p');
  newP.textContent = '"' + randomQuote.quote + '" — ' + randomQuote.author;
  document.querySelector('.quote-content').appendChild(newP);
}
...
function showRiddle() {
  ...
  questionP.textContent = randomRiddle.question;
  const answerP = document.createElement('p');
  answerP.textContent = randomRiddle.answer;
  answerP.style.display = 'none';
  answerP.classList.add('riddle-answer');
...
function revealAnswers() {
  ...
  if (!answer) {
    alert('There is no riddle! Click "Riddle me" first.');
